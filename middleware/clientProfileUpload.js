// middleware/clientProfileUpload.js
...
// Configure storage for client profile images
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const dir = "uploads/clients/profiles";
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, `client-profile-${req.user.id}-${uniqueSuffix}${ext}`);
  },
});
...
// Set up multer middleware for client profile image uploads
exports.uploadClientProfileImage = multer({
  storage: storage,
  limits: {
...
exports.handleClientProfileUploadErrors = (err, req, res, next) => {
...
exports.processClientProfileData = (req, res, next) => {
  try {
    ...
    if (req.file) {
      req.body.profile_img = `/uploads/clients/profiles/${req.file.filename}`;
    } else if (existing_profile_img && !req.body.profile_img) {
      // Keep the existing profile image if no new one is uploaded
      req.body.profile_img = existing_profile_img;
    }
